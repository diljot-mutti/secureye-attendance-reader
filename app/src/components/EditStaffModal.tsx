...
export default function EditStaffModal({ isOpen, onClose, onUpdateStaff, staff }: EditStaffModalProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setNotification({ type: "success", message: "Staff member updated successfully!" });
      ...
      setNotification({
        type: "error",
        message: error instanceof Error ? error.message : "Failed to update staff member. Please try again.",
      });
    } finally {
      setIsLoading(false);
    }
  ...
  return (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 w-full max-w-md">
        <h2 className="text-xl font-semibold mb-4 text-gray-900">Edit Staff Member</h2>
        <form onSubmit={handleSubmit}>
          ...
          <div className="flex justify-end space-x-4">
            ...
            >
              {isLoading ? (
                <>
                  <span className="opacity-0">Update Staff</span>
